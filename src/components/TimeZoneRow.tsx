...
const TimeZoneHeaderCell: React.FC<{
  ...
  dragHandleProps?: any;
          ...
          <td className={`sticky left-0 z-20 px-1.5 sm:px-3 py-1.5 sm:py-2.5 align-top border-r border-gray-100 min-w-[100px] max-w-[220px] w-auto whitespace-nowrap truncate transition-colors group-hover:bg-primary-50 ${isHome ? 'bg-gray-100' : 'bg-white'}`}>
    <div className="flex items-start space-x-1 sm:space-x-2">
      ...
      <span className="text-lg sm:text-xl flex-shrink-0" role="img" aria-label={`Flag of ${location.timezone.country}`}>
      ...
      <div className="flex flex-col min-w-0 flex-1">
        ...
        <div className="flex items-center min-w-0">
          ...
          >
            <svg className="w-3 sm:w-4 h-3 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div className="flex items-center text-[10px] sm:text-xs text-gray-500 whitespace-nowrap overflow-hidden">
          <span className="truncate max-w-[120px] sm:max-w-[140px]">{location.timezone.country}</span>
  ...
  return (
    <tr 
      ...
      className={`group hover:bg-primary-50 transition-colors ${
        isHomeRow 
          ? 'bg-gray-100 border-l-8 border-l-primary-600 shadow-md' 
          : ''
